 ...
 * 转换器工厂
 */
public class SiteDoorPointQueryConvertFactory {
    ...
    static {
            ...
            if(((String)t).contains(",")){
                ...
                for (String s : split) {
                	if(StringUtils.isNotBlank(s)&&s.length()>1) {
                		list.add(new EsMatchPhrasePrefixQueryDTO("name",s));
                	}
                }
                for (String s : split) {
                	if(StringUtils.isNotBlank(s)&&s.length()>1) {
            	...
            	if(StringUtils.isNotBlank((String)t) && t.toString().length()>1) {
            		list.add(new EsMatchPhrasePrefixQueryDTO("name",t));
    	...
    	QUERY_MAP.put("fuzzyKey",t->{
	        ...
	        keys.add("name");
	        keys.add("address");
    		return new EsMultiQueryDTO(keys, (String) t,80);
        });
		...
		QUERY_MAP.put("type",t->new EsMatchQueryDTO("type",t));
    ...
    public static QueryConvert getConvert(){
        return new SiteDoorPointQueryConvert(QUERY_MAP,SORT_MAP);
    }
